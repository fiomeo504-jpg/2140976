...
CATEGORIES = ["Еда", "Транспорт", "Развлечения", "Коммунальные услуги", "Одежда", "Здоровье", "Другое"]
...
class ExpenseTracker:
    ...
    def create_widgets(self):
        ...
        add_frame = tk.LabelFrame(self.root, text="Добавить расход", padx=15, pady=10)
        add_frame.pack(fill="x", padx=10, pady=5)

        # Сумма
        tk.Label(add_frame, text="Сумма (₽):").grid(row=0, column=0, sticky="e", padx=5, pady=5)
        self.amount_entry = tk.Entry(add_frame, width=15)
        self.amount_entry.grid(row=0, column=1, padx=5, pady=5)

        # Категория
        tk.Label(add_frame, text="Категория:").grid(row=0, column=2, sticky="e", padx=5, pady=5)
        self.category_var = tk.StringVar(value=CATEGORIES[0])
        self.category_combo = ttk.Combobox(add_frame, textvariable=self.category_var, values=CATEGORIES, width=18)
        self.category_combo.grid(row=0, column=3, padx=5, pady=5)

        # Дата
        tk.Label(add_frame, text="Дата (ГГГГ-ММ-ДД):").grid(row=0, column=4, sticky="e", padx=5, pady=5)
        ...
        tk.Button(add_frame, text="➕ Добавить расход", command=self.add_expense,
                  bg="lightgreen", font=("Arial", 10)).grid(row=0, column=6, padx=15, pady=5)
        ...
        # Фильтр по категории
        tk.Label(filter_frame, text="Категория:").grid(row=0, column=0, sticky="e", padx=5, pady=5)
        ...
        tk.Button(filter_frame, text="🔍 Применить фильтр", command=self.apply_filter,
                  bg="lightblue").grid(row=0, column=6, padx=5, pady=5)
        tk.Button(filter_frame, text="❌ Сбросить фильтр", command=self.reset_filter,
                  bg="lightgray").grid(row=0, column=7, padx=5, pady=5)

        # === Рамка подсчёта суммы ===
        sum_frame = tk.LabelFrame(self.root, text="Подсчёт расходов", padx=15, pady=10)
        sum_frame.pack(fill="x", padx=10, pady=5)

        tk.Label(sum_frame, text="Период от:").grid(row=0, column=0, padx=5, pady=5)
        self.sum_date_from = tk.Entry(sum_frame, width=12)
        self.sum_date_from.grid(row=0, column=1, padx=5, pady=5)

        tk.Label(sum_frame, text="до:").grid(row=0, column=2, padx=5, pady=5)
        self.sum_date_to = tk.Entry(sum_frame, width=12)
        self.sum_date_to.grid(row=0, column=3, padx=5, pady=5)

        tk.Button(sum_frame, text="💰 Рассчитать сумму", command=self.calculate_sum,
                  bg="lightyellow").grid(row=0, column=4, padx=10, pady=5)

        self.sum_label = tk.Label(sum_frame, text="Общая сумма: 0.00 ₽", font=("Arial", 12, "bold"), fg="green")
        self.sum_label.grid(row=0, column=5, padx=20, pady=5)
